...
class L3VXLANTunnelHS(tunnel_mode.TunnelMode):
    ...
    def add_slice_to_overlay(self, overlayid, overlay_name, routerid,
        ...
        for subnet in subnets:
            ...
            # Get hub
            overlay = srv6_sdn_controller_state.get_overlay(
                overlayid, tenantid)
            hub = overlay['hub']
            vtep_ip = self.get_vtep_ip(
            ...
            if response != SbStatusCode.STATUS_SUCCESS:
                # If the operation has failed, report an error message
                logger.warning('Cannot set route for %s in %s (out '
                               'interface %s)' % (subnet, hub, vtep_name))
                return NbStatusCode.STATUS_INTERNAL_SERVER_ERROR
        # Success
    ...
    def create_tunnel(self, overlayid, overlay_name,
        ...
        vtep_ip_hub = self.get_vtep_ip(hub, tenantid)
        # get VNI
        vni = srv6_sdn_controller_state.get_vni(
            overlay_name, tenantid)
        if vni is None:
    ...
    def init_hub(self, overlayid, overlay_name,
        ...
        vxlan_port_site = srv6_sdn_controller_state.get_tenant_vxlan_port(
            tenantid)
        # get WAN interface
        wan_intf_hub = srv6_sdn_controller_state.get_wan_interfaces(
            deviceid, tenantid)[0]
        # get VNI for the overlay
        vni = srv6_sdn_controller_state.get_vni(
            overlay_name, tenantid)
        ...
        # crete VTEP interface
        response = self.srv6_manager.createVxLAN(
            deviceid, self.grpc_client_port,
            ifname=vtep_name,
        ...
        if response != SbStatusCode.STATUS_SUCCESS:
            ...
            logger.warning('Cannot create VTEP %s in %s'
                           % (vtep_name, deviceid))
            return NbStatusCode.STATUS_INTERNAL_SERVER_ERROR
        devices_in_vrf.append(vtep_name)
        hub_ip = self.get_vtep_ip(
            deviceid, tenantid)
        # Set VTEP IP address
        response = self.srv6_manager.create_ipaddr(
            deviceid, self.grpc_client_port,
    ...
    def remove_slice_from_overlay(self, overlayid, overlay_name, routerid,
        ...
        for subnet in subnets:
            ...
            if gateway is not None and gateway != '':
                ...
                if response != SbStatusCode.STATUS_SUCCESS:
                    ...
                    logger.warning('Cannot remove route for %s (gateway %s) '
                                   'in %s ' % (subnet, gateway, mgmt_ip_site))
                    return NbStatusCode.STATUS_INTERNAL_SERVER_ERROR
            # Remove the subnet from the hub
            #
            # get VNI
            vni = srv6_sdn_controller_state.get_vni(
                overlay_name, tenantid)
            ...
            if response != SbStatusCode.STATUS_SUCCESS:
                # If the operation has failed, report an error message
                logger.warning('Cannot remove route for %s in %s (out '
                               'interface %s)' % (subnet, hub, vtep_name))
                return NbStatusCode.STATUS_INTERNAL_SERVER_ERROR
        # retrive VRF name
        vrf_name = 'vrf-%s' % (tableid)
    ...
    def destroy_hub(self, overlayid, overlay_name,
        ...
        response = self.srv6_manager.remove_ipaddr(
            deviceid, self.grpc_client_port,
            ip_addr=hub_ip, device=vtep_name)
        if response != SbStatusCode.STATUS_SUCCESS:
            # If the operation has failed, report an error message
            logger.warning('Cannot remove IP %s for VTEP %s in %s'
                           % (hub_ip, vtep_name, deviceid))
            return NbStatusCode.STATUS_INTERNAL_SERVER_ERROR
        # remove VTEP interface
        response = self.srv6_manager.delVxLAN(
            deviceid, self.grpc_client_port,
            ifname=vtep_name,
        )
...
class IPv4VXLANTunnelHS(L3VXLANTunnelHS):
...
class IPv6VXLANTunnelHS(L3VXLANTunnelHS):
