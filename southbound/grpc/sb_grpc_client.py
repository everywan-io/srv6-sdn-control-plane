...
class SRv6ExplicitPathHandler:
    ...
    def get_grpc_session(self, ip_address, port, secure):
        ...
        if secure:
            ...
            channel = grpc.secure_channel("%s:%s" % (ip_address, port),
                                          grpc_client_credentials)
        else:
            channel = grpc.insecure_channel("%s:%s" % (ip_address, port))
        return (srv6_explicit_path_pb2_grpc
                .SRv6ExplicitPathStub(channel), channel)
    ...
    def addFromJson(self, data):
        ...
        for data in json_data:
            ...
            srv6_stub, channel = self.get_grpc_session("localhost",
                                                       12345, SECURE)
            path_request = srv6_explicit_path_pb2.SRv6EPRequest()
...
class SRv6SouthboundVPN:
    ...
    def get_vpns(self, ip_address):
        ...
        try:
            # Get the reference of the stub
            srv6_stub, channel = self.get_grpc_session(ip_address,
                                                       12345, SECURE)
            # Get VPNs
            ...
            if status_code.value[0] == 14:  # Unavailable
                print ("Cannot establish a connection "
                       "with gRPC server %s-%s" % (ip_address, 12345))
                print "%s: %s" % (status_code.name, e.details())
                return None

    def create_vpn(self, ip_address, vpn_name, tableid, sid):
        # Create the request
    ...
    def add_local_interface_to_vpn(self, ip_address, vpn_name, ifname):
        # Create the request
    ...
    def remove_local_interface_from_vpn(self, ip_address, vpn_name, ifname):
        # Create the request
    ...
    def remove_vpn(self, ip_address, vpn_name, tableid, sid):
        # Create the request
...
class InterfaceManager:
    ...
    def add_ip_address_to_interface(self, ip_address, ifname, ip):
        # Create the request
        request = interface_manager_pb2.AddIPAddressRequest()
        request.ifname = ifname
        request.ipaddr = ip
        # Get the reference of the stub
        srv6_stub, channel = self.get_grpc_session(ip_address, 12345, SECURE)
        # Remove the VPNs
        response = srv6_stub.AddIPAddressToInterface(request)
        # Let's close the session
        channel.close()
        # Return the response
        return response.message

    def remove_ip_address_from_interface(self, ip_address, ifname, ip):
        # Create the request
        request = interface_manager_pb2.RemoveIPAddressRequest()
        request.ifname = ifname
        request.ipaddr = ip
        # Get the reference of the stub
        srv6_stub, channel = self.get_grpc_session(ip_address, 12345, SECURE)
        # Remove the VPNs
        response = srv6_stub.RemoveIPAddressToInterface(request)
        # Let's close the session
        channel.close()
        # Return the response
        return response.message
...
if __name__ == "__main__":
    ...
    (srv6VPNHandler
     .addLocalInterfaceToVPN("2000::1", "vpn_a", ["ads1-eth1", "ads1-eth2"]))
    (srv6VPNHandler
     .addRemoteInterfaceToVPN("2000::1",
                              ["fdf0:0:0:4::/64"], "2", "fdff::2", "1111::"))
    (srv6VPNHandler
     .removeLocalInterfaceFromVPN("2000::1", ["ads1-eth1", "ads1-eth2"]))
    (srv6VPNHandler
     .removeRemoteInterfaceFromVPN("2000::1", ["fdf0:0:0:4::/64"], "2"))
    ...
    (srv6ExplicitPathHandler
     .add("3333:4::2/128", "eth0", ["3333:3::2", "3333:2::2", "3333:1::2"]))
